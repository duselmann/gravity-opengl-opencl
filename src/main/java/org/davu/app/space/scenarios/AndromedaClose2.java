...
public class AndromedaClose2 extends Galaxies2 {
	...
	public void makeVertices(VaoVboManager manager) {
        ...
        for(int b=2; b<getParticleCount(); b++) {
            ...
            float massOfAllStars = leftRightRatio*getParticleCount()*massBase;
            float galaxyArea = (float)Math.PI * galaxyRadius * galaxyRadius;
            float starInnerArea = (float)Math.PI * r*r;
            float ratioMassInnerToOuterStars = starInnerArea / galaxyArea;
            float innerStarMass = massOfAllStars * ratioMassInnerToOuterStars;
            vr = Math.sqrt(innerStarMass/r)/1f; // galactic radial velocity from other stars   r r / rm rm
